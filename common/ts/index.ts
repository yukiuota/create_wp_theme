...
document.addEventListener('DOMContentLoaded', () => {
  const fileInput = document.getElementById('batchFileInput') as HTMLInputElement;
  if (fileInput) {
...
function updateFileList(): void {
  ...
  const convertBtn = document.getElementById('convertBatchBtn') as HTMLButtonElement;
  
  if (!fileListElement || !convertBtn) return;
...
function convertHtmlToPhp(htmlContent: string): string {
...
// グローバルスコープに新しい関数を公開
(window as any).convertBatch = convertBatch;
